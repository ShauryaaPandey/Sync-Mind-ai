...
export const MeetingDetails = () => {
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'tasks' | 'decisions' | 'transcript'>('summary');
  ...
  useEffect(() => {
    ...
    return () => {
      if (id) socketService.leaveMeeting(id);
    };
  }, [id]);

  if (loading) return <div className="min-h-screen bg-gray-900 text-white p-8">Loading meeting...</div>;
  if (!meeting) return <div className="min-h-screen bg-gray-900 text-white p-8">Meeting not found.</div>;
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6 max-w-5xl mx-auto">
      ...
      <div className="flex border-b border-gray-700 mb-6 gap-6">
        {[
          ...
          { id: 'tasks', label: `Action Items (${meeting.actionItems?.length || 0})`, icon: CheckSquare },
      ...
      {activeTab === 'tasks' && (
        <TaskChecklist meetingId={meeting._id} initialTasks={meeting.actionItems} />
      )}

      {activeTab === 'decisions' && (
        <div className="space-y-3">
          {meeting.keyDecisions?.map((decision: string, index: number) => (
            <div key={index} className="p-4 bg-gray-800 border border-gray-700 rounded-lg text-gray-200">
              💡 {decision}
            </div>
          ))}
        </div>
